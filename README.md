...
**DISCLAIMER: buttons don't work in firefox, the backend request get block by the browser with ``ns_binding_aborted``**
...
- TransactionLog, keeps track of the flow of money and items
- ShoppingBasket, gives the users a way to buy multiple items
- TypeComments, keeps track of comments made on items, also allows for tree like comments
...
The HTTP module that is included in go is extensive and provide large about of the functionally needed for any web application.
...
Another thing with the sql transactions in go/mysql, is that an transactions only can have one rows active at the same time.
for example this would throw an error (note that this is not valid go as error handling is omitted)

```go
t := db.Begin()

rows := t.Query("SELECT * FROM myTable")

for row := rows.Next() {
    t.Exec("SOMETHING")
    // will trow error since we are still reading myTable
}
```

This forces us to save the result from MyTable in an temporary variable and then loop over that later.
